...
FORWARD_CHANNEL_ID = -1026477495
CHANNEL_ID = -1026477495
error_channel_id = -1026477495
...
def is_user_admin(user_id, chat_id):
    # Grant admin access to the bot owner directly by user ID
    if user_id == 1026477495:  
        return True
...
def approve_or_disapprove_user(message):
    ...
    if not is_admin:
        bot.send_message(chat_id, "*bhag betichod papa ka pass*", parse_mode='Markdown')
        return
...
def attack_command(message):
    ...
    try:
        user_data = users_collection.find_one({"user_id": user_id})
        if not user_data or user_data['plan'] == 0:
            bot.send_message(chat_id, "*papa bolo. Please contact the administrator.*", parse_mode='Markdown')
            return
...
def handle_message(message):
    if message.text == "Instant Plan 🧡":
        ...
        bot.send_message(message.chat.id, "*FOR RESSELER SHIP DM :-@Devil_4413*", parse_mode='Markdown')
